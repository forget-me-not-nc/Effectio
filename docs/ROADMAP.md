...
| 5 | ~~**`entity.GetStatusStackCount(key)` shortcut.**~~ **Shipped:** `IEffectioEntity.GetStatusStackCount(string)` + new `EffectioEntity(string id, IStatusEngine engine)` ctor + `EffectioManager.CreateEntity` wires it automatically. Manually-constructed entities using the v1.0 single-arg ctor return 0 (documented fallback). | Today consumers have to reach into `manager.Statuses.GetStacks(...)`. Pure ergonomics. | XS |
| 6 | ~~**Per-stack expiration semantics: test + docs.** Decide whether stacks expire all-at-once or one-at-a-time, lock with tests, document.~~ **Shipped:** locked down v1.x combined-counter behaviour with 6 regression tests in `StackExpirationTests` + XML docs on `IStatus.Duration` / `IStatusEngine.ApplyStatus` + CHANGELOG "Stack-expiration contract" section. Per-stack independent expirations remain a v2 candidate that can opt in without breaking the v1 contract. | Current behaviour is an implementation detail. | S |
...
| 8 | ~~**`Conditional` effect action.** `.WhenStatBelow(stat, value, then, else)` without writing a custom action.~~ **Dropped.** Existing `Triggered` effect type already covers the once-when-condition-becomes-true case; per-tick re-evaluation in Periodic / Aura effects is achievable via `WithAction(IEffectAction)` with a custom predicate. The `else` branch on a Periodic effect creates surprising fire-every-tick behaviour. Sugar value did not justify the design + test + doc cost. | Sugar for "heal if low HP, otherwise damage". | S |
| 9 | ~~**Stack-aware ticks.** `StatusBuilder.OnTick(effect).PerStack()` so `Bleeding x3` ticks 3 x -1 HP.~~ **Shipped as `StatusBuilder.TicksPerStack()`** - per status rather than per effect, because the fluent shape `OnTick(e).PerStack()` needs an effect handle the builder does not hand out, and a status wanting half its ticks scaled is describing two statuses. Scaling is repetition, not multiplication: an arbitrary `IEffectAction` cannot be multiplied. Shipped alongside `StackDecay`, which answers the question this task implies but never asked - whether stacks fall off together or one at a time. | Companion to task #2; status-side counterpart. | S |
| 10 | ~~**README: registration-order rule + reaction priority.** Document the gotcha until task #3 ships.~~ **Shipped:** "Which reaction wins" section under Reactions - worked example of a five-condition reaction preempting its two-condition subset, the same-priority-fires-in-registration-order rule stated plainly, and the more-conditions-means-higher-priority heuristic. | Cheap, prevents foot-guns for early users. | XS |
| 11 | ~~**Comprehensive benchmark matrix.** Cross-product of {10, 50, 100, 500} entities x {10, 50, 100} stats x {10, 50, 100} effects x {10, 50, 100} reactions.~~ **Shipped (scope-trimmed):** the original cross-product was overdesigned (4 x 3 x 3 x 3 = 108 benchmark cells, ~3-4 hours of measurement per run, most cells redundant with each other). Delivered instead: `StatusEngineBenchmark` (ApplyStatus refresh paths + Tick at 128 / 1024 entities), `StackAwareReactionBenchmark` (RequireStacks / ConsumesStacks vs presence at 10 / 50 / 100 reactions), and `docs/PERF.md` with reference numbers + allocation contract + how-to-run + per-PR perf review checklist. `StatusOnRefreshBenchmark` deferred to a follow-up after v1.1 #7 (`OnRefresh`) merges. End-to-end frame-budget headroom ("does Tick complete inside 16 ms at scale N?") covered by the existing `ManagerTickBenchmark`. | Today's benchmarks are per-engine micro-benches; we have no end-to-end tick budget data at realistic game scale. | L |
